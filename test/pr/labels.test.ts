...
describe('Label Functions', () => {
    ...
    describe('getLabelColor', () => {
        it('should return the correct color for each label', () => {
            expect(labels.getLabelColor(RhdaLabels.RHDA_SCAN_PASSED)).toBe(
                '0E8A16',
            );
            expect(labels.getLabelColor(RhdaLabels.RHDA_SCAN_FAILED)).toBe(
                'E11D21',
            );
            expect(labels.getLabelColor(RhdaLabels.RHDA_FOUND_WARNING)).toBe(
                'EE9900',
            );
            expect(labels.getLabelColor(RhdaLabels.RHDA_FOUND_ERROR)).toBe(
                'B60205',
            );
    ...
    describe('getLabelDescription', () => {
        it('should return the correct description for each label', () => {
            expect(
                labels.getLabelDescription(RhdaLabels.RHDA_SCAN_PASSED),
            ).toBe('RHDA found no vulnerabilities');
            expect(
                labels.getLabelDescription(RhdaLabels.RHDA_SCAN_FAILED),
            ).toBe('RHDA scan failed unexpectedly');
            expect(
                labels.getLabelDescription(RhdaLabels.RHDA_FOUND_WARNING),
            ).toBe(`RHDA found 'warning' level vulnerabilities`);
            expect(
                labels.getLabelDescription(RhdaLabels.RHDA_FOUND_ERROR),
            ).toBe(`RHDA found 'error' level vulnerabilities`);
    ...
    describe('createRepoLabels', () => {
        it('should create missing labels in the repository', async () => {
            ...
            const actionsOctokitMock = new actionsOctokit({
                auth: utils.getGhToken(),
            });
    ...
    describe('cleanupLabels', () => {
        it('should remove existing RHDA labels from the PR', async () => {
            ...
            expect(ghCore.info).toHaveBeenCalledWith(
                `Removing labels "${RhdaLabels.RHDA_SCAN_FAILED}", "${RhdaLabels.RHDA_SCAN_PASSED}", "${RhdaLabels.RHDA_FOUND_WARNING}", "${RhdaLabels.RHDA_FOUND_ERROR}" from pull request`,
            );
            labels.repoLabels.forEach((label) => {
                expect(octokitMock.request).toHaveBeenCalledWith(
    ...
    describe('addLabelsToPr', () => {
        it('should add labels to the PR', async () => {
            ...
            expect(ghCore.info).toHaveBeenCalledWith(
                `Adding labels "label1", "label2" to pull request`,
            );
            expect(octokitMock.request).toHaveBeenCalledWith(
