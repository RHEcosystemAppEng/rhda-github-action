...
import { executeDockerImageAnalysis, DockerImageAnalysis } from '../src/imageAnalysis';
...
describe('executeDockerImageAnalysis', () => {
    ...
    it('should execute Docker image analysis and return JSON report', async () => {
        const mockImageAnalysisResult = { report: 'Example Image Analysis report' };

        vi.mocked(exhortServices.imageAnalysisService).mockResolvedValue(JSON.stringify(mockImageAnalysisResult))
