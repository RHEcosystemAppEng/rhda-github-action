...
export async function handleSarif(rhdaReportJson, manifestFilePath, ecosystem, sha, ref, analysisStartTime, prData) {
    ...
    const rhdaReportSarifFilePath = path.resolve(".", `${ghCore.getInput(Inputs.RHDA_REPORT_NAME)}.sarif`);
    ...
    if (uploadSarif) {
        ...
        await upload.uploadSarifFile(githubToken, rhdaReportSarifFilePath, analysisStartTime, sha, ref, github.context.repo, printSecurityTabLink);
    }
    else {
