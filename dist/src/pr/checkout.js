...
export async function getOriginalCheckoutBranch() {
 ...
 */
export async function checkoutPr(baseRepoUrl, prNumber) {
...
export async function checkoutCleanup(prNumber, originalCheckoutBranch) {
    const remoteName = getPRRemoteName(prNumber);
...
function getPRRemoteName(prNumber) {
    return `remote-${prNumber}`;
}
function getPRBranchName(prNumber) {
    return `pr-${prNumber}`;
}
