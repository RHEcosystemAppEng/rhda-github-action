...
async function run() {
    ...
    const rhdaReportJson = await generateRHDAReport(manifestFilePath, ecosystem);
    /* Save RHDA report to file */
    const rhdaReportJsonFilePath = path.resolve(".", `${ghCore.getInput(Inputs.RHDA_REPORT_NAME)}.json`);
    ...
    if (constants.vulnerabilitySeverityOrder[vulSeverity] > 0) {
        if (failOn !== "never") {
            if (failOn === "warning") {
                ghCore.info(`Input "${Inputs.FAIL_ON}" is "${failOn}", and at least one warning was found. Failing workflow.`);
                ...
                ghCore.info(`Input "${Inputs.FAIL_ON}" is "${failOn}", and at least one error was found. Failing workflow.`);
