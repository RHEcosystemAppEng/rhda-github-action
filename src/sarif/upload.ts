...
export async function uploadSarifFile(
    ...
    if (printSecurityTabLink) {
        ...
        const codeScanningUrl =
            utils.getEnvVar('GITHUB_SERVER_URL') +
            `/${owner}/${repo}/security/code-scanning?${search.toString()}`;

        ghCore.info(
            `👀 Review the Code Scanning results in the Security tab: ${codeScanningUrl}`,
        );
    }
}
...
async function waitForUploadToFinish(
    ...
    while (uploadStatus !== 'complete') {
        ...
        if (tries > maxTries) {
            throw new Error(
                `SARIF upload timed out: status was ${uploadStatus} after ${timeout / 1000}s.`,
            );
        }
